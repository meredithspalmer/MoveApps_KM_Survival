...
summary_table <- events_with_ind |>
  ...
  summarise(first_timestamp = min(as.Date(timestamp), na.rm = TRUE),
            ...
            animal_birth_hatch_year = if ("animal_birth_hatch_year" %in% names(events_with_ind))
              str_c(unique(animal_birth_hatch_year[!is.na(animal_birth_hatch_year)]), collapse = " | ")
            else NA_character_,
